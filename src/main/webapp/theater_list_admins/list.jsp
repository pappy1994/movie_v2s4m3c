...
function create_theater_form(theaterno) {

    var url = './create.do?theaterno=' + theaterno; 
...
function update_form(theaterno, areano) {
    ...
    var url = './update.do?theaterno=' + theaterno;
    var win = window.open(url, '카테고리 그룹 수정', 'width=800px, height=400px');
...
function delete_form(theaterno, areano) {
    ...
    var url = './delete.do?theaterno=' + theaterno;
    alert(url);
...
<TABLE class='table table-striped'>
  ...
  <c:forEach var="theaterVO" items = "${list }">
    ...
    <TR>
      <TD style='text-align: center;'>${theaterVO.theaterno }</TD>
...
<button type='button' id='create_form' onclick="create_theater_form(${theaterVO.theaterno});">상영 극장 등록</button>
