...
<form name='frm' id='frm' method='get' action='./list_by_areano_search_paging.do'>
    ...
    <ASIDE style='float: left;'>
      ...
      <c:if test="${param.theater.length() > 0 }">
        > [${param.theater }] 검색 목록 (${search_count } 건)
      </c:if>
    </ASIDE>
    <ASIDE style='float: right;'>
      ...
      <c:choose>
        <c:when test="${param.theater != '' }">
          <input type='text' name='theater' id='theater' value='${param.theater }' 
                     style='width: 35%;'>
        </c:when>
        <c:otherwise>
          <input type='text' name='theater' id='theater' value='' style='width: 35%;'>
        </c:otherwise>
      </c:choose>
      <button type='submit'>검색</button>
      <c:if test="${param.theater.length() > 0 }">
        <button type='button' 
                     onclick="location.href='./list_by_areano_search_paging.do?areano=${areaVO.areano}'">검색 취소</button>  
      </c:if>
    </ASIDE> 
  </form>
...
<TABLE class='table table-striped'>
  ...
  <tbody>
  <c:forEach var="theaterVO" items = "${list_by_areano_search_paging }">
    <c:set var="theaterno" value="${theaterVO.theaterno }" />
    
    <TR>
      <TD style='text-align: center;'>${theaterVO.theater }</TD>
      <TD style='text-align: center;'>
        <A href="javascript: update_form(${theaterno },${theaterVO.areano },${nowPage });"><IMG src='./images/update.png' title='수정'></A>
        <A href="javascript: delete_form(${theaterno },${theaterVO.areano },${nowPage });"><IMG src='./images/delete.png' title='삭제'></A>
      </TD>
    </TR>
  </c:forEach> 
  </tbody>
